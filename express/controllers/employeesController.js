...
const createNewEmployee = (req, res) => {
    const type = typeof(data.employees.find(employee => employee.id === parseInt(req.body.id)));
    

    const newEmployee = {
        id: type !== 'undefined' ? (data.employees[data.employees.length - 1].id) + 1 : req.body.id,
...
const updateEmployee = (req, res) => {
    ...
    data.setEmployees(unsortedArray.sort((a,b) => a.id > b.id ? 1 : a.id < b.d ? -1 : 0));
    res.json(data.employees);
}
